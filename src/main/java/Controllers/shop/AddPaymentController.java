        ...
        try {
            ...
            try {
                if (!shopService.getShopItemById(shopItemId).isPresent()) {
                    showAlert("Article introuvable", "L'article avec l'ID " + shopItemId + " n'existe pas dans la base de données", Alert.AlertType.ERROR);
                    return false;
                }
            } catch (Exception e) {
                showAlert("Erreur de vérification", "Impossible de vérifier l'existence de l'article: " + e.getMessage(), Alert.AlertType.ERROR);
                return false;
            }
