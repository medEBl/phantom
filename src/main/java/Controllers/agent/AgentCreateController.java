...
public class AgentCreateController {
    ...
    private void loadDynamicData() {
        try {
            ...
            // 1. Load Players (Users with PLAYER role)
            if (cbPlayer != null) {
                try {
                    List<User> playerUsers = userService.getUsersByRole("PLAYER");
                    for (User user : playerUsers) {
                        String displayName = user.getFullName() != null && !user.getFullName().isEmpty()
                                ? user.getFullName() + " (" + user.getUsername() + ")"
                                : user.getUsername();
                        playerMap.put(displayName, user.getId());
                        cbPlayer.getItems().add(displayName);
                    }
                } catch (Exception e) {
    ...
    private void handleCreate() {
        ...
        System.out.println("DEBUG: Starting handleCreate");
        ...
        System.out.println("DEBUG: pseudo='" + pseudo + "'");
        System.out.println("DEBUG: rank='" + rank + "'");
        System.out.println("DEBUG: socials='" + socials + "'");
        System.out.println("DEBUG: game='" + game + "'");
    ...
    private void navigateBack() {
        try {
            String fxmlPath = this.isAdminMode ? "/agent/fxml/ListAgentsBack.fxml" : "/agent/fxml/ListAgents.fxml";
